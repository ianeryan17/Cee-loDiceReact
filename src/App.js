...
function App() {
  ...
  const [diceFaces, setDiceFaces] = useState(["/1_dot.png", "/1_dot.png", "/1_dot.png"]);
...
//   /* BUTTON FUNCTIONS */  
  ...
  const updateStake = (newStake) => {
    console.log("update stake called, stake: ", newStake);
  ...
  const animation = useSpring({
    ...
    fontSize: isVisible ? '50px' : '0px', // Set font size based on visibility
    config: { duration: 300 }
  });
...
/* RESULT STATES */
  ...
  return (
    <div style={{
      ...
      bottom: 0,
      left: 0,
      backgroundImage: `url('/concrete.png')`, // Apply background image to the game container
      ...
      <div style={{ position: 'absolute', top: 20, left: 20 }}>
        <span style={{ fontSize: '100px', color: 'black' }}>Money = </span>
        <input
          style={{
            fontSize: '100px',
            color: 'black',
